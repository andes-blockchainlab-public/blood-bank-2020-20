...
export const sendBlockchain = async (
  namespace: string,
  method: string,
  payload
): Promise<void> => {
  const transactionsT: any[] = []
  for (const obj of payload) {
    payload = { ...payload, lastUpdated: new Date() }
    payload = { namespace, Method: method, payload: { ...obj } }
    const payloadBytes = cbor.encode(payload)

    const transactionHeaderBytes = protobuf.TransactionHeader.encode({
      familyName: 'bloodbank',
      familyVersion: '1.0',
      inputs: [INT_KEY_NAMESPACE], //Lectura
      outputs: [INT_KEY_NAMESPACE], //Escritura
    ...
    transactionsT.push(transaction)
  }
